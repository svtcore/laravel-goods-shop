...
@section('title', __('admin_user_orders.title'))
@section('content')
<h4 class="category_header">
    @if ($users->count() > 0)
    {{ $users[0]->user_fname }} {{ $users[0]->user_lname }} {{ __('admin_user_orders.header')}}
    <br />
    <small>{{ $users[0]->user_phone }}</small>
    @endif
</h4>
<div class="container">
    ...
    <table class="table table-hover sortable-theme-ligh" data-sortable>
        <thead>
            <tr>
                <th scope="col" class="small-column text-center">{{ __('admin_user_orders.order_num_tb')}}</th>
                <th scope="col" class="w_7 text-center">{{ __('admin_user_orders.date_tb')}}</th>
                <th scope="col" class="w_15 text-center">{{ __('admin_user_orders.address_tb')}}</th>
                <th scope="col" class="small-column text-center">{{ __('admin_user_orders.price_tb')}}</th>
                <th scope="col" class="small-column text-center">{{ __('admin_user_orders.payment_tb')}}</th>
                <th scope="col" class="small-column text-center">{{ __('admin_user_orders.status_tb')}}</th>
                <th scope="col" data-sortable="false" class="small-column text-center">{{ __('admin_user_orders.action_tb')}}</th>
            </tr>
        </thead>
        <tbody id="content-data">
            ...
            <tr>
                ...
                <td>
                    <form action="{{ route('admin.orders.delete', ['id' => $order->order_id]) }}" method="POST">
                        <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                            <div class="btn-group mr-2" role="group" aria-label="First group">
                                <a class="btn btn-primary" href="{{ route('admin.orders.show', ['id' => $order->order_id]) }}" role="button" target="_blank" type="button">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-arrow-up-right" viewBox="0 0 16 16">
                                        <path fill-rule="evenodd" d="M8.636 3.5a.5.5 0 0 0-.5-.5H1.5A1.5 1.5 0 0 0 0 4.5v10A1.5 1.5 0 0 0 1.5 16h10a1.5 1.5 0 0 0 1.5-1.5V7.864a.5.5 0 0 0-1 0V14.5a.5.5 0 0 1-.5.5h-10a.5.5 0 0 1-.5-.5v-10a.5.5 0 0 1 .5-.5h6.636a.5.5 0 0 0 .5-.5z" />
                                        <path fill-rule="evenodd" d="M16 .5a.5.5 0 0 0-.5-.5h-5a.5.5 0 0 0 0 1h3.793L6.146 9.146a.5.5 0 1 0 .708.708L15 1.707V5.5a.5.5 0 0 0 1 0v-5z" />
                                    </svg>
                                </a>
                                <a class="btn btn-primary" href="{{ route('admin.orders.edit', ['id' => $order->order_id]) }}" role="button" target="_blank" type="button">
            ...
            <tr>
                <td colspan="7" class="text-center">
                    <h5>{{ __('admin_user_orders.not_found')}}</h5>
                </td>
            </tr>
            @endif
            @endforeach
