...
@section('title',  __('admin_category_edit.title'))
@section('content')
<div class="container">
<form action="{{ route('admin.category.update', ['id' => $category->catg_id]) }}" method="POST">
@method('PUT')
@csrf
<table class="table data_small_table">
  <thead>
    <tr>
      <th scope="col"><h4 class="category_header">{{ __('admin_category_edit.header') }}</h4></th>
    </tr>
  </thead>
  <tbody>
        ...
        <nav>
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                  <label for="inputAddress2">{{ __('admin_category_edit.name') }}</label>
                  <input type="text" class="form-control" name="category_name_en" placeholder="English name" value="{{$category->catg_name_en}}"/>
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                  <label for="inputAddress2">{{ __('admin_category_edit.name') }}</label>
                  <input type="text" class="form-control" name="category_name_de" placeholder="Deutsch name" value="{{ $category->catg_name_de }}">
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                  <label for="inputAddress2">{{ __('admin_category_edit.name') }}</label>
                  <input type="text" class="form-control" placeholder="назва українською" name="category_name_uk" value="{{ $category->catg_name_uk }}">
            ...
            <div class="form-row">
                <div class="form-group col-md-12">
                  <label for="inputAddress2">{{ __('admin_category_edit.name') }}</label>
                  <input type="text" class="form-control" placeholder="название на русском" name="category_name_ru" value="{{ $category->catg_name_ru }}">
            ...
            <button  class="btn btn-primary w-100" type="submit" >{{ __('admin_category_edit.update') }}</button>
        </td>
    </tr>
  </tbody>
</table>
