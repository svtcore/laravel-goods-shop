...
@section('title', __('manager_order_show.title'))
@section('content')
<h4 class="category_header">
    {{ __('manager_order_show.title')}}
</h4>
<div class="container">
...
<table class="table">
  <thead>
    <tr>
      <th scope="col" class="small-column">{{ __('manager_order_show.order_num_tb')}}</th>
      <th scope="col" class="small-column">{{ __('manager_order_show.date_tb')}}</th>
      <th scope="col" class="w-25">{{ __('manager_order_show.customer_info_tb')}}</th>
      <th scope="col" class="w-25">{{ __('manager_order_show.ordered_tb')}}</th>
      <th scope="col" class="small-column">{{ __('manager_order_show.price_tb')}}</th>
      <th scope="col" class="small-column">{{ __('manager_order_show.payment_tb')}}</th>
      <th scope="col" class="small-column">{{ __('manager_order_show.status_tb')}}</th>
      <th scope="col" class="small-column">{{ __('manager_order_show.action_tb')}}</th>
    </tr>
  </thead>
  <tbody id="content-data">
    @if(isset($order->order_id))
            ...
            <div class="list-group w-100 user_orders_info_block"  d="collapse_{{ $order->order_id }}">
                <span class="list-group-item list-group-item-action flex-column w-100 align-items-start active">
                    <div class="d-flex w-50 justify-content-between">
                        <h5 class="mb-1">{{ __('manager_order_show.name')}}</h5>
                    </div>
                ...
                <span class="list-group-item list-group-item-action flex-column w-100 align-items-start active">
                    <div class="d-flex w-50 justify-content-between">
                        <h5 class="mb-1">{{ __('manager_order_show.phone') }}</h5>
                    </div>
                ...
                <span class="list-group-item list-group-item-action flex-column w-100 align-items-center active">
                    <div class="d-flex w-50 justify-content-between">
                        <h5 class="mb-1">{{ __('manager_order_show.code')}}</h5>
                    </div>
                ...
                <span class="list-group-item list-group-item-action flex-column w-100 align-items-center active">
                    <div class="d-flex w-50 justify-content-between">
                        <h5 class="mb-1">{{ __('manager_order_show.address')}}</h5>
                    </div>
                ...
                <span class="list-group-item list-group-item-action flex-column w-100 align-items-center active">
                    <div class="d-flex w-50 justify-content-between">
                        <h5 class="mb-1">{{ __('manager_order_show.note') }}</h5>
                    </div>
            ...
            @endif
                ...
                <td>
                        <form class="form-group w-100" action="{{ route('manager.order.update', ['id' => $order->order_id ]) }}" method="POST">
                            @method('PUT')
                            @csrf
                                <input type="hidden" name="type" value="created"/>
                                <input class="btn btn-primary w-100" name="submit" type="submit" value="{{ __('manager_order_show.created') }}">
                        </form>
                        <form class="form-group w-100" action="{{ route('manager.order.update', ['id' => $order->order_id ]) }}" method="POST">
                            @method('PUT')
                            @csrf
                                <input type="hidden" name="type" value="confirm"/>
                                <input class="btn btn-primary w-100" name="submit" type="submit" value="{{ __('manager_order_show.processing') }}">
                        </form>
                        <form class="form-group w-100" action="{{ route('manager.order.update', ['id' => $order->order_id ]) }}" method="POST">
                            @method('PUT')
                            @csrf
                                <input type="hidden" name="type" value="complete"/>
                                <input class="btn btn-primary w-100" name="submit" type="submit" value="{{ __('manager_order_show.completed') }}">
                        </form>
                        <form class="form-group w-100" action="{{ route('manager.order.update', ['id' => $order->order_id ]) }}" method="POST">
                            @method('PUT')
                            @csrf
                            <input type="hidden" name="type" value="cancel"/>
                            <input class="btn btn-primary w-100" name="submit" type="submit" value="{{ __('manager_order_show.canceled') }}">
                        </form>
                        <form class="form-group w-100">
                            <a class="btn btn-primary w-100" href="{{ route('manager.order.edit', ['id' => $order->order_id ]) }}" target="_blank" role="button">{{ __('manager_order_show.edit') }}</a>
                        </form>
                    </td>
            </tr>
    @else
